...
public class BTree<T extends Comparable<T>> implements ITree<T> {
    ...
    public static int comparisonCount = 0; // Лічильник порівнянь
...
/*Перевірка на порожнє дерево: Якщо дерево порожнє (тобто root == null),
створюється новий кореневий вузол з одним ключем, який зберігає значення value.
* */
        if (root == null) {
            root = new Node<T>(null, maxKeySize, maxChildrenSize);
            root.addKey(value);
        } else {
            // Проходимо по дереву, щоб знайти правильний листовий вузол для нового значення
            Node<T> node = root;
            while (node != null) {
                if (node.numberOfChildren() == 0) {
                    // Якщо ми знайшли листовий вузол (немає дітей)
                    node.addKey(value);// Додаємо значення до ключів цього вузла
                    if (node.numberOfKeys() <= maxKeySize) {
                        // Якщо після додавання кількість ключів в межах допустимого, завершимо
                        break;
                    }
                    // Якщо кількість ключів перевищує maxKeySize, розділяємо вузол
                    split(node);
                    break;
                }
                // Навігація по вузлах

                // Якщо значення менше або дорівнює першому ключу в вузлі
                ...
                // Якщо значення більше за останній ключ вузла
                int numberOfKeys = node.numberOfKeys();
                int last = numberOfKeys - 1;
                T greater = node.getKey(last);
                if (value.compareTo(greater) > 0) {
                    node = node.getChild(numberOfKeys);
                    // Переходимо до крайнього правого дочірнього вузла
                    continue;
                }

                // Пошук внутрішніх вузлів між ключами
                for (int i = 1; i < node.numberOfKeys(); i++) {
                    T prev = node.getKey(i - 1);
                    T next = node.getKey(i);
                    // Якщо значення між двома ключами, переходимо до відповідного дочірнього вузла
                    if (value.compareTo(prev) > 0 && value.compareTo(next) <= 0) {
                        node = node.getChild(i);
                        // Переходимо до середнього дочірнього вузла
                        break;
                    }
                }
            }
    ...
    private void split(Node<T> nodeToSplit) {
        // Отримуємо вузол, який потрібно розділити
        Node<T> node = nodeToSplit;
        int numberOfKeys = node.numberOfKeys();// Кількість ключів у вузлі
        int medianIndex = numberOfKeys / 2;// Індекс середнього елемента
        T medianValue = node.getKey(medianIndex);// Середнє значення, яке підніметься вгору

        // Створюємо лівий дочірній вузол і додаємо до нього ключі з лівої половини вузла
        Node<T> left = new Node<T>(null, maxKeySize, maxChildrenSize);
        for (int i = 0; i < medianIndex; i++) {
            left.addKey(node.getKey(i));// Додаємо ключі до лівого вузла
        }

        // Якщо у вузла є діти, додаємо дочірні вузли до лівого нового вузла
        if (node.numberOfChildren() > 0) {
        ...
        // Створюємо правий дочірній вузол і додаємо до нього ключі з правої половини вузла
        Node<T> right = new Node<T>(null, maxKeySize, maxChildrenSize);
        for (int i = medianIndex + 1; i < numberOfKeys; i++) {
            right.addKey(node.getKey(i));// Додаємо ключі до правого вузла
        }

        // Якщо у вузла є діти, додаємо дочірні вузли до правого нового вузла
        if (node.numberOfChildren() > 0) {
        ...
        // Якщо вузол не має батьківського (він кореневий), створюємо новий корінь
        if (node.parent == null) {
            // new root, height of tree is increased
            Node<T> newRoot = new Node<T>(null, maxKeySize, maxChildrenSize);
            newRoot.addKey(medianValue);// Додаємо середнє значення до нового кореня
            node.parent = newRoot;
            root = newRoot;
            node = root;
            // Додаємо лівий і правий дочірні вузли до нового кореня
            node.addChild(left);
            node.addChild(right);
        } else {
            // Якщо вузол має батька, піднімаємо середнє значення до батьківського вузла
            Node<T> parent = node.parent;
            parent.addKey(medianValue);// Додаємо середнє значення до батька
            parent.removeChild(node);// Видаляємо поточний вузол з дітей батьківського вузла
            parent.addChild(left);// Додаємо лівий дочірній вузол до батьківського вузла
            parent.addChild(right);// Додаємо правий дочірній вузол до батьківського вузла

            // Якщо кількість ключів у батька перевищує допустиме значення, розділяємо батька
    ...
    public T remove(T value) {
        T removed = null; // Ініціалізуємо змінну для збереження видаленого значення
        Node<T> node = this.getNode(value);// Знаходимо вузол, який містить значення для видалення
        removed = remove(value, node);// Викликаємо метод для видалення значення з вузла
        return removed;// Повертаємо видалене значення
    }
    ...
    private T remove(T value, Node<T> node) {
        // Якщо вузол відсутній, повертаємо null (значення не знайдено)
        if (node == null) return null;

        T removed = null;
        // Знаходимо індекс значення у вузлі
        int index = node.indexOf(value);
        // Видаляємо ключ з вузла і зберігаємо видалене значення
        removed = node.removeKey(value);
        if (node.numberOfChildren() == 0) {// Якщо це листовий вузол (не має дітей)
            // leaf node
            if (node.parent != null && node.numberOfKeys() < minKeySize) {
                // Якщо вузол має батька і менше мінімальної кількості ключів
                this.combined(node);// Комбінуємо вузол з сусіднім, щоб підтримати баланс
            } else if (node.parent == null && node.numberOfKeys() == 0) {
                // Якщо це корінь і він порожній
                root = null;// Видаляємо корінь, встановлюючи його як null
            }
        } else {
            // Якщо це внутрішній вузол (має дочірні вузли)
            Node<T> lesser = node.getChild(index);// Отримуємо дочірній вузол, що знаходиться ліворуч від значення
            Node<T> greatest = this.getGreatestNode(lesser);// Знаходимо найбільше значення у піддереві
            T replaceValue = this.removeGreatestValue(greatest);// Видаляємо найбільше значення з піддерева
            node.addKey(replaceValue);// Замінюємо видалене значення новим значенням з піддерева
            if (greatest.parent != null && greatest.numberOfKeys() < minKeySize) {
                this.combined(greatest);// Комбінуємо, якщо після видалення порушений баланс
            }
            if (greatest.numberOfChildren() > maxChildrenSize) {
                this.split(greatest);// Розділяємо вузол, якщо він має більше максимально допустимої кількості дочірніх елементів
            }
        }

        size--;// Зменшуємо розмір дерева, оскільки елемент був видалений
    ...
    public T get(T keyNode) {
        Node<T> node = getNode(keyNode);
        if (node != null) {
            // Знайдено вузол, повертаємо значення
            int index = node.indexOf(keyNode);
            if (index != -1) {
                T foundNode = node.getKey(index);
                return foundNode; // Повертаємо значення
            }
        }
    ...
    private Node<T> getNode(T value) {
        ...
        while (node != null) {
            // Виконуємо бінарний пошук у вузлі
            int index = binarySearch(node, value);
            comparisonCount++;

            if (index != -1) {
                comparisonCount++; // Збільшуємо лічильник на 1, якщо знайшли ключ
                return node; // Знайдено вузол
            }

            // Якщо ключ не знайдено, переходимо до дочірніх вузлів
            int numberOfKeys = node.numberOfKeys();
            comparisonCount++; // Збільшуємо лічильник на 1, якщо знайшли ключ

            if (value.compareTo(node.getKey(0)) < 0) {
                node = node.getChild(0); // Перейти до першого дочірнього вузла
                comparisonCount++; // Збільшуємо лічильник на 1, якщо знайшли ключ
            } else if (value.compareTo(node.getKey(numberOfKeys - 1)) > 0) {
                node = node.getChild(numberOfKeys); // Перейти до останнього дочірнього вузла
                comparisonCount++; // Збільшуємо лічильник на 1, якщо знайшли ключ
            } else {
    ...
    private int binarySearch(Node<T> node, T key) {
        int left = 0;
        int right = node.numberOfKeys() - 1;

        while (left <= right) {
            int mid = left + (right - left) / 2;
            T midValue = node.getKey(mid);
            comparisonCount++; // Збільшуємо лічильник на 1, якщо знайшли ключ

            if (midValue.compareTo(key) == 0) {
                return mid; // Знайдено ключ
            } else if (midValue.compareTo(key) < 0) {
                left = mid + 1; // Шукати праворуч
            } else {
                right = mid - 1; // Шукати ліворуч
            }
        }
